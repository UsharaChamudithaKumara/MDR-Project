...
import { Form, Input, Select, DatePicker, Button, Upload, message, Typography, Space, AutoComplete, Steps, Checkbox } from "antd";
...
function EditMDR() {
  ...
  useEffect(() => {
    const fetchMDR = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/mdr/${id}`);
        const { header, items: mdrItems, attachments } = res.data;

        // Populate Form
        form.setFieldsValue({
          po_number: header.po_number,
          supplier_type: header.supplier_type,
          supplier_name: header.supplier_name,
          supplier_ref: header.supplier_ref,
          grn_no: header.grn_no,
          mdr_date: header.mdr_date ? dayjs(header.mdr_date) : null,
          inspection_by: header.inspection_by,
          department: header.department,
          corrective_action: header.corrective_action ? header.corrective_action.split(", ") : [],
          status: header.status
        });

        // Populate Items
        if (mdrItems && mdrItems.length > 0) {
          setItems(mdrItems.map(item => ({
            ...item,
        ...
        if (attachments && attachments.length > 0) {
          setFileList(attachments.map(att => ({
            ...
            name: att.file_name,
            status: 'done',
            url: `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}${att.file_path}`,
          })));
        }
      } catch (error) {
        console.error("Failed to fetch MDR", error);
        message.error("Failed to load MDR details.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      const res = await axios.put(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/update-mdr/${id}`, formData, {
        headers: {
          "Content-Type": "multipart/form-data"
        }
      });

      message.success(`MDR Updated Successfully`);
      navigate(`/mdr/${id}`);
    } catch (error) {
      console.error(error);
    ...
    <div className="font-sans fade-in pb-12">
      <div className="flex items-center mb-8">
        ...
        <div className="flex-1 flex justify-between items-center">
          <div>
            <Title level={2} className="m-0 text-slate-800 tracking-tight" style={{ marginBottom: 0 }}>Edit MDR</Title>
            <Text className="text-slate-500 font-medium text-base">Update the details of the material discrepancy report</Text>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex justify-center items-center py-20">
          <div className="text-lg text-slate-500">Loading MDR...</div>
        </div>
      ) : (
